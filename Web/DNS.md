...
> [!TIP] 
> **Subdominio vs Vhost:** Un subdominio permite organizar diferentes secciones de un sitio web, cada subdominio tiene un registro DNS y puede estar en la misma IP que el dominio principal o no. Un Vhost es una configuración dentro del servidor que permite alojar múltiples sitios web o aplicaciones en un único servidor, pudiendo tener o no registro DNS.
> 

> [!TIP]
> **La cabecera Host:** En esencia, la cabecera `Host` funciona como un interruptor, permitiendo al servidor web determinar dinámicamente qué sitio web servir basándose en el nombre de dominio solicitado por el navegador.
...
4️⃣ Los TLD servers le darán la lista de `Authoritative Server`s y sus IPs, `ejemplo.com. NS ns1.proveedor.com.` según su dominio, Ej `.es` para España
...
> Gestionan un TLD concreto (`.com`, `.org`, `.net`, `.es`... Tenemos verisign para `.com`) . No conocen las IPs de los dominios, pero conocen a que Authoritative Nameserver hacerle la petición para cada dominio registrado bajo ese TLD. Por ejemplo para`google serían` serían `ns1.google.com, ns2.google.com...`. Cuando se registra un dominio con por ejemplo GoDaddy, se notifica al operador del TLD los nameservers del dominio.
...
- El `IN` indica que es un registro para internet, es una convención,
...
Cada registro WHOIS suele contener la siguiente información:
- **Domain Name:** El propio nombre de dominio (p. ej., example.com)
- **Registrar**: La empresa donde se registró el dominio (p. ej., GoDaddy, Namecheap) 
- **Registrant Contact**: La persona u organización que registró el dominio. 
- **Administrative Contact**: La persona responsable de la gestión del dominio.
- **Technical Contact**: La persona que se ocupa de los problemas técnicos relacionados con el dominio.
- **Creation and Expiration Dates**: Cuándo se registró el dominio y cuándo está previsto que expire.
- **Name Servers**: Servidores DNS. 
- **Location**: dirección del titular del registro. Indicio de sospecha: país con una alta prevalencia de ciberdelincuencia.
...
| Herramienta    | Uso                                                                                                                                                                      |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `nslookup`     | Permite consultas DNS simples (registros A, AAAA y MX) para verificaciones rápidas                                                                                       |
| `host`         | Herramienta de búsqueda de DNS optimizada para busquedas rápidas                                                                                                         |
| `dnsenum`      | Herramienta automatizada de enumeración de DNS, ataques de diccionario (dictionary attacks), fuerza bruta (brute-forcing), transferencias de zona (si están permitidas). |
| `fierce`       | Herramienta de reconocimiento de DNS y enumeración de subdominios con búsqueda recursiva y detección de comodines (wildcard detection).                                  |
| `dnsrecon`     | Enumeración completa de DNS, identificando subdominios y recopilando registros DNS para un análisis posterior.                                                           |
| `theHarvester` | Herramienta de OSINT que recopila información de diversas fuentes, incluyendo registros DNS (direcciones de correo electrónico).                                         |
